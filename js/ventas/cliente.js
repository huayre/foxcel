		...
		submitHandler       : function(form){
				...
				if(modo=='insertar'){
					...
					$("#VentanaTransparente").css("display","none");
						alert('Se ha ingresado un cliente.');
						location.href = base_url+"index.php/ventas/cliente/clientes";
					});	
					...
					url = base_url+"index.php/ventas/cliente/modificar_cliente";
					$.post(url,dataString,function(data){
						$("#VentanaTransparente").css("display","none");
...
function eliminar_cliente(cliente){
	if(confirm('Esta seguro desea eliminar este cliente?')){
		...
		url = base_url+"index.php/ventas/cliente/eliminar_cliente";
		$.post(url,dataString,function(data){
			if ( data == 0 ){
				alert('Existen comprobantes relacionados a este cliente, por ello no puede ser eliminado.');
			}
			else{
				url = base_url+"index.php/ventas/cliente/clientes";
				location.href = url;
...
function ver_cliente(cliente){
	url = base_url+"index.php/ventas/cliente/ver_cliente/"+cliente;
	$("#zonaContenido").load(url);
}
function atras_cliente(){
	location.href = base_url+"index.php/ventas/cliente/clientes";
}
        ...
        $.getJSON(url,function(data){
                ...
                /*$.each(data,function(i,item){
                        ...
                        $("#empresa_persona").val(item.codigo); 
                         $("#empresa_msg").html("<b>Se ha encontrado una empresa con el número de "+tipodesc+ " indicado. <a href='#' style='background: #ffffcc; color: #666666; text-decoration: none; padding: 4px;font-size: 10pt;font-weight: bold' onClick='editar_cliente("+item.cod_cliente+")' >Click Aqui</a></b>");
...
function buscar_persona_nofunciona_aler(){
