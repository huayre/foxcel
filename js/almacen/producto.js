...
jQuery(document).ready(function(){
    ...
    $("#addCompatible").click(function(){
            if ($("#seleccionar_articulos").val() !== ''){
                if ($("#descripcion_producto").text() == "")
                    articulos = $("#seleccionar_articulos").val();
                else
                    articulos = $("#descripcion_producto").text() + ", " +$("#seleccionar_articulos").val();

                articulos = articulos.toUpperCase();

                $("#descripcion_producto").text(articulos);
                $("#seleccionar_articulos").val('');
            }
            else
                alert('Debe agregar un valor.');
        });

        $("#remCompatible").click(function(){
            var text = $("#descripcion_producto").text();
            ...
            for ( i = 0; i < size; i++){
                ...
                if (i == 0)
                    ...
                    articulos = articulos + ", " + values[i];
            }
            $("#descripcion_producto").text(articulos);
        });

        $("#remAllCompatible").click(function(){
            seguro = confirm('¿Esta seguro de limpiar el campo de compatibilidad?');
            if (seguro == true)
                $("#descripcion_producto").text('');
        });
...
function asignar_codigo_usuario() {
    var url = base_url + "index.php/almacen/producto/asignar_codigo_usuario";
        $.ajax({
            type: 'POST',
            url: url,
            dataType: 'json',
