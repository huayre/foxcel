...
<script>
  $(document).ready(function(){
    ...
    $("#reporteExcelMarca").click(function(){
      if($('#fecha_inicio').val() == "" || $('#fecha_fin').val() == ""){
        alert("Ingrese ambas fechas");
      }
      else{
        ...
        if (startDate > endDate){
          ...
          location.href = "<?=base_url();?>index.php/reportes/ventas/filtroMarcaExcel/"+fechaIF;
        }
      }
    });
  });
...
<div id="pagina">
    <div id="zonaContenido">
		<div align="center">
    <div id="tituloForm" class="header">REPORTES DE VENTAS POR MARCA</div>
    <div id="frmBusqueda">
      <form method="post" action="" id="generar_reporte">
        Desde: <input type="text" id="fecha_inicio" name="fecha_inicio" readonly class="cajaMedia fecha" value="<?php echo ((isset($_POST['reporte'])) ? $_POST['fecha_inicio'] : ''); ?>"> Hasta: <input type="text" id="fecha_fin" name="fecha_fin" class="cajaMedia fecha" readonly value="<?php echo ((isset($_POST['reporte'])) ? $_POST['fecha_fin'] : ''); ?>"> <input type="hidden" name="reporte" value=""><input type="button" id="reporte" value="Generar" class="btn btn-success">
        <a href="javascript:;" id="reporteExcelMarca">
          <img src="<?php echo base_url();?>images/xls.png" style="width:40px; border:none;" class="imgBoton" align="absmiddle"/>
        </a>
      </form>
      ...
      <br>
        ...
        <thead>
          <tr class="cabeceraTablaResultado"><th colspan="2">Reporte de ventas por MARCA desde <?php echo $fecha_inicio; ?> hasta el <?php echo $fecha_fin; ?></th></tr>
          <tr class="cabeceraTabla"><th>MARCA</th><th> Ventas S/.</th></tr>
        </thead>
        <tbody>
        <?php $total = 0; ?>
        ...
        <?php foreach($resumen as $fila): ?>
          ...
          <?php echo "<tr><td>{$fila['NOMBRE']}</td><td>".number_format($fila['VENTAS'],2,'.','')."</td>"; ?>
        <?php endforeach; ?>
        </tbody>
          <tfoot><tr class="cabeceraTabla"><th>TOTAL</th><th><?php echo number_format($total,2,'.',''); ?></th></tr></tfoot>
      </table>
      ...
      <?php
        ...
        $months  = getMonths($fecha_inicio,$fecha_fin)+1;
      ?>
      <table class="fuente8 display" cellspacing="0" cellpadding="3" border="0" id="resumenMensual">
        <thead>
          ...
          <th rowspan="2" valign="bottom">MARCA</th><?php 
      ...
      <tbody> <?php
        ...
        foreach($mensual as $fila):
          ...
          foreach($keys as $key):
            ...
            echo "<td>".number_format($fila[$key],2,'.','')."</td>";
            else
            echo "<td>{$fila[$key]}</td>";
          endforeach;
          echo "</tr>";
      ...
      <tfoot>
        <tr class="cabeceraTabla"><th>TOTAL</th>
      ...
      <thead>
        <tr class="cabeceraTablaResultado"><th colspan="<?php echo (1+(1+($anioFin-$anioInicio))); ?>" align="center">Detalle Anual</th></tr>
        <tr class="cabeceraTabla">
        <th>MARCA</th>
        <?php 
          for($i = $anioInicio; $i<=$anioFin;$i++):
      ...
      <tfoot>
        <tr class="cabeceraTabla"><th>TOTALES</th>
        <?php foreach($sumas as $suma):?>
          <?php echo '<th>'.number_format($suma,2,'.','').'</th>'; ?>
        <?php endforeach; ?>
        </tr>
