                        ...
                        <table class="fuente8 display" id="table-pagos">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:32%;" data-orderable="true">ESTADO</th>
                                </tr>
                            </thead>
...
<div id="modal_cancelar_pago" class="modal fade" role="dialog">
    <div class="modal-dialog w-30">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title text-center" style="color: #0d95e8;font-size: 20px" >CANCELAR PAGO </h4>

            </div>
            <div class="modal-body">
                <form method="POST" id="formcancelarpago">

                    <label>Ingrese el motivo</label>
                    <textarea class="form-control" style="width: 90%;text-align: center" rows="4" name="motivocancelar" id="motivocancelar"></textarea>

                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-success" onclick="cancelarpago()">Aceptar</button>
                <button type="button" class="btn btn-default" data-dismiss="modal">Salir</button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
            ...
            success: function(data){
                if (data.match == true){
                    ...
                    $.each(pagos, function(i,item){
                        if (item.estado==5) {
                            tr = '<tr style="background-color:#F5B7B1">';
                            tr += '<td>' + item.fecha + '</td>';
                            ...
                            tr += '<td>' + item.btncancelar + '</td>';
                            tr += '</tr>';
                            $("#table-pagos .pagos-info").append(tr);
                        }
                        else{
                            tr = '<tr style="background-color: #A3E4D7">';
                            tr += '<td>' + item.fecha + '</td>';
                            tr += '<td>' + item.serieNumero + '</td>';
                            tr += '<td>' + item.moneda + '</td>';
                            tr += '<td>' + item.monto + '</td>';
    ...
    function table_cuentas(id = ""){
        $('#table-cuentasPendientes').DataTable({
            filter: false,
            destroy: true,
            processing: true,
            serverSide: true,
    ...
    function modalcancelarpago(codigopago,serieNumero){
        $('#codigopago').remove();
        $('#motivocancelar').val('')

        $('#modal_cancelar_pago').modal('show');
        let imputcodigopago='<input type="hidden" id="codigopago" name="codigopago" value="'+codigopago+'"/>'
        $('#formcancelarpago').append(imputcodigopago);
        serienumero='<h4>'+serieNumero+'</h4>'
        $('#serienumeromodal').append(serieNumero);


    }
    function cancelarpago(){
        if ($('#motivocancelar').val() == ''){
            Swal.fire({
                icon: "info",
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: '<?=base_url();?>index.php/tesoreria/cuentas/darBajaPago/',
                data:$('#formcancelarpago').serialize(),
                success: function(data){
                    if (data.result == "success"){
                        Swal.fire({
                            icon: "success",
                            title: "El pago fue cancelado correctamente...!!",
                            html: "<b class='color-red'></b>",
                            showConfirmButton: true,
                            timer: 2000
                        });
                        $("#modal_cancelar_pago").modal("hide");
                        $("#modal_pagos").modal("hide");
