        ...
        if ( count($listado) > 0 ){
            foreach ($listado as $indice => $valor) {
                ...
                $divNPC = '<div style="text-align: left;">
                                <div class="costo"> <img src="'.base_url().'images/icono_nuevo.png" height="14px"/>
                                    <span class="editar_costo">
                                        <form action="'.base_url().'" method="POST">
                                            <input type="number" class="cajaMedia" placeholder="Precio costo" id="'.$posPrecio.'" name="'.$posPrecio.'" step="0.01" min="0.01" value="'.$valor->PROD_CostoPromedio.'"/>
                                ...
                                <span style="color:red; font-weight: bold" id="span'.$posPrecio.'" name="span'.$posPrecio.'">'.number_format($valor->PROD_CostoPromedio,2).'</span>
                            </div>';
                $posDT=0;            
                $lista[$indice] = array(
                    ...
                    $posDT++ => $valor->PROD_DescripcionBreve,
                    $posDT++ => $divNPC
                );
    ...
    public function autocompletado_producto_x_nombre()
    {
        ...
        if($cargarProductos != NULL){
            foreach($cargarProductos AS $productos => $value){
                ...
                $result[] = array("value" => $nombPro, "codigo" => $codPro, "codinterno" => $codProUsuario, "pcosto" => $costoPro, "stock" => $stock,"flagGenInd" => $value->PROD_GenericoIndividual);
            }
        }
        echo json_encode($result);
    }
    ...
    public function autocompletado_producto_x_codigo()
    {
        ...
        if($query != NULL){
            ...
            foreach($query AS $producto => $value){
                $result[] = array("value" => $value->PROD_CodigoUsuario ,"nombre" => $value->PROD_Nombre, "codigo" => $value->PROD_Codigo, "codinterno" => $value->PROD_CodigoUsuario);
            }
...
//        $this->cezpdf->ezText('', '', array("leading" => 100));
        ...
        $this->cezpdf->ezText('USUARIO: '.$this->nombre_persona, 8, array("leading" => 30, 'left' => 28));
...
//        $this->cezpdf->ezText('', '', array("leading" => 100));
        ...
        $this->cezpdf->ezText('USUARIO: '.$this->nombre_persona, 8, array("leading" => 30, 'left' => 90));
        $this->cezpdf->ezText('', '', array("leading" => 10));
...
//        /* Listado de detalles */
    ...
    public function asignar_codigo_usuario(){
       
       
        $CodUsu = $this->producto_model->obtener_ult_id();

        if ($CodUsu[0]->PROD_CodigoUsuario == NULL){
            $preCod = "COD";
            $nvoCod = "01";
        }
        else{
            $preCod = substr($CodUsu[0]->PROD_CodigoUsuario,0,3);
            $nvoCod = substr($CodUsu[0]->PROD_CodigoUsuario,4) + 1;
            $nvoCod = "0".$nvoCod;
        }

        $codigo_usuario = $preCod.$nvoCod;
        
        $json = array("match" => true, "codigo_usuario" => $codigo_usuario);
        
        echo json_encode($json);
    }

    public function productos_compatibles($codigo = NULL){
        if($codigo != NULL){
            $compatibles = $this->producto_model->listar_compatibles($codigo);
