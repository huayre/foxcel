...
class Guiarem extends controller{
    ...
    public function datatable_guiarem($tipo_oper = 'V'){
        ...
        if (count($listado) > 0) {
            foreach ($listado as $indice => $valor) {
                ...
                $lista[] = array(
                                    0 => $fecha,
                                    1 => $serie,
                                    2 => $this->getOrderNumeroSerie($numero),
                                    3 => $nombre,
                                    4 => $boleta,
                                    5 => $comprobante,
                                    6 => $orden_compra,
                                    7 => $valor->DOCUP_Codigo, # EN ESTE CAMPO GUARDA LA OC

                                    8 => $view_estado,
                                    9 => $eliminar,
                                    10 => "<div align='center' class='editar_data_$item'>$editar</div>", # $editar,
                                    11 => $pdfImprimir,
                                    12 => $pdfImprimir2,
                                    13 => $excel,
                                    14 => "<div align='left' class='pdfSunat_$item'>
                                                <span class='icon-loading'></span>
                                                <span class='pdfSunat_data_$item'>$pdfSunat</span>
                                            </div>", #$pdfSunat,
                                    15 => $PDFCompromiso,
                                    16 => "<div align='center' class='disparador_$item'>
                                                <span class='icon-loading'></span>
        ...
        $json = array(
                            ...
                            "recordsTotal"    => count($this->guiarem_model->getGuiaRemision($filterAll)),
                            "recordsFiltered" => intval( count($this->guiarem_model->getGuiaRemision($filter)) ),
                            "data"            => $lista
                    );
    ...
    public function nueva($tipo_oper = 'V'){
        ...
        $data_confi1 = $this->configuracion_model->obtener_numero_documento($this->somevar['compania'], $tipo, $tipo_oper);
        ...
        */
        if ($tipo_oper == 'V') {
            ...
            $cofiguracion_datos = $this->configuracion_model->obtener_numero_documento($compania, $tipo,'V');
            $data['tipo_codificacion'] = $data_confi_docu[0]->COMPCONFIDOCP_Tipo;
    ...
    public function editar($codigo, $tipo_oper = 'V'){
        ...
        if ($cliente != '' && $cliente != '0') {
            ...
            if ($datos_cliente) {
                ...
                $ruc_cliente = ( $ruc == NULL || $ruc == "" || $ruc == 0 ) ? $dni_cliente : $ruc;
    ...
    public function envioGuiaSunat($codigo) {
        ...
        $cliente_tipoDoc = (strlen($ruc_cliente) < 10) ? '1' : '6' ;
