...
class Comprobante extends Controller{
    ...
    public function datatable_comprobantes($tipo_oper = '', $tipo_docu = ''){
            ...
            foreach ($listado_comprobantes as $indice => $valor) {
                ...
                if ( $rol == 1 || $_SESSION['user_name'] == 'ccapasistemas' ){
                    if ( $estado == 1 || $estado == 2 ){
                        if ( $this->fecha_limite($fechaLimite) == true && $estado == 1 || $tipo_oper == 'C' && $estado == 1 || $tipo_docu == 'N' && $estado == 1 ){
                        	if($nota==null){
                            	$img_estado = "<a href='".base_url()."index.php/seguridad/usuario/ventana_confirmacion_usuario2/$serie/$codigo/$tipo_oper/$tipo_docu"."' data-fancybox data-type='iframe'> <img src='" . base_url() . "images/active.png' alt='Activo' title='Activo'/></a>";
                            }
                          }
                        else
                            $img_estado = "";
                ...
                if ($estado == 1)
                    ...
                    if ($estado == '2'){
                        ...
                        #$editar = ($valor->sin_lote == 0 && $tipo_oper == "V") ? $editar : "";
                    }
                ...
                if ($estado == '1' && $tipo_oper == 'V') {
                    if($tipo_docu != 'N'){
                        ...
                        $xml = "<a href='".$this->consutarRespuestaXmlsunat($codigo,false)."' target='_parent' style='cursor:pointer;'><img src='" . base_url() . "images/xml.png' width='16' height='16' border='0' title='XML Sunat'></a>";
                    }
                    ...
                    #$pdfCompromiso = "";#"<a href='javascript:;' onclick='comprobante_ver_pdf_compromiso(" . $codigo .",".$ConversorDeNumero.",".$imp.",".$tipo_oper2.")'  target='_parent'><img src='" . base_url() . "images/compromiso.png' width='16' height='16' border='0' title='Ver Compromiso'></a>";
                ...
                if ($estado == 0){
                    ...
                    $img_estado = "<div> <span class='detallesAnulado'> <img src='".base_url()."images/inactive.png' alt='Anulado' title='Anulado'/> <span class='detallesAnulado2'><b>DETALLES:</b> $motivo </span> </span> </div>";
                    $editar = "";
                    $pdfCompromiso = "";
                    $xml = "<a href='".$this->consutarRespuestaXmlsunat($codigo,false)."' target='_parent' style='cursor:pointer;'><img src='" . base_url() . "images/xml.png' width='16' height='16' border='0' title='XML Sunat'></a>";
                }
                ...
                $lista[] = array(
                    ...
                    $posDT++ => $pdfImprimir,
                    $posDT++ => $pdfImprimir2,
                    $posDT++ => "<div align='center' class='enviarcorreo_data_$item'>$enviarcorreo</div>",#"<div align='center' class='pdfSunat_$item'> <span class='icon-loading'></span> <span class='pdfSunat_data_$item'>$pdfSunat</span> </div>",
                    $posDT++ => "<div align='center' class='disparador_$item'> <span class='icon-loading'></span> <span class='disparador_data_$item'>$disparador</span> </div>" . $canjeHTML,#$xml,
                    $posDT++ => "",#"<div align='center' class='compromiso_data_$item'>$pdfCompromiso</div>",
                    $posDT++ => "",
                    $posDT++ => "<b>".$valor->vendedor."</b>"
);
                $contadoVacios = 1;
                $item++;
            }
        ...
        $json = array(
                            ...
                            "recordsTotal"    => count($this->comprobante_model->getComprobantes($filterAll)),
                            "recordsFiltered" => intval( count($this->comprobante_model->getComprobantes($filter)) ),
                            "data"            => $lista
                    );
    ...
    public function comprobante_insertar(){
        ...
        $configuracion_datos = $this->configuracion_model->obtener_numero_documento($compania, $tipo); // Vuelve a consultar el ultimo correlativo - Rawil
    ...
    public function insertar_guiarem($comprobante){
        ...
        #####################################
            ...
            $configuracion_datos = $this->configuracion_model->obtener_numero_documento($compania, 10,$tipo_oper);
    ...
    public function disparador($tipo_oper = 'V', $codigo, $tipo_docu = 'F'){
        ...
        if ($lotesAsignados == true){
            ...
            if (($comprobante->CPC_TipoDocumento == "F" || $comprobante->CPC_TipoDocumento == "B") && $_SESSION["empresa"] == 1 )
                $aceptada = true; //$this->envioSunatFac($codigo, $comprobante);


            if ($aceptada == true || $comprobante->CPC_TipoDocumento == 'N' || $_SESSION["empresa"] > 1){
    ...
    public function downloadsFiles($inicio, $fin){
        ...
        for ($i = $inicio; $i <= $fin; $i++){
            ...
            $serieFac = "BPP1";//boletas de la 3 a la 10
            $tipo_de_comprobante = '2';
   ...
   /**gcbq json verificacion de cantidad por producto y cantidad por serie este metodo lo usa json de inventario guiarem y otros***/
    ...
    function obtener_datos_cliente($cliente, $tipo_docu = 'F')
    {
        ...
        if ($tipo == 0) {
            ...
            $nombre = $datos_empresa[0]->comprobante_insertar_refEMPRC_RazonSocial;
    ...
    public function ver_reporte_pdf_commpras($anio)
    {
        ...
        $this->cezpdf->ezText('USUARIO: '.$this->nombre_persona ."  ", 8);
    ...
    public function estadisticas_compras_ventas_mensual_excel($tipo, $anio, $mes){
		...
		<tr style="font-weight:bold;font-size:10pt"><td colspan="11">USUARIO: '.$this->nombre_persona.'</td> </tr>
    ...
    public function estadisticas_compras_ventas_mensual($tipo, $anio, $mes){
        ...
        $cabeceraHTML = '<table>
                            <tr>
                                <td style="font-weight:bold;font-size:10pt">USUARIO:'.$this->nombre_persona.' </td>                              
    ...
    public function verPdf($tipo_oper = '', $tipo_docu = '',$dataEviar=""){
        ...
        elseif($tipo_docu=='N'){
         ...
         $options2 = array("leading" => 15, "left" => 30);
            ...
            $this->cezpdf->ezText('USUARIO: '.$this->nombre_persona, 7, array("left" =>30));
