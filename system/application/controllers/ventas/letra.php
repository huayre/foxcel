...
class Letra extends Controller {
        ...
        if ( $letra != NULL ){
            foreach ($letra as $indice => $val) {
                $lista = array(
                                    ...
                                    "serie_letra" => $val->LET_Serie,
                                    "numero_letra" => $val->LET_Numero,
                                    "cod_comprobante" => $val->CPP_Codigo
                                );
            }
    ...
    public function actualizar_letras(){
        ...
        $let_serie = $this->input->post("let_serie");
        $let_numero = $this->input->post("let_numero");
        $comprobante = $this->input->post("cod_comprobante");
        ...
        if ($letra != ""){ 
            ...
            $info = new stdClass();
            $info->importe = $importe[$i];
            $info->caja = $caja;
            $info->observacion = $observacion[$i];
            $info->MONED_Codigo = $moneda;
            $info->FORPAP_Codigo = $forma_pago;
            $info->let_serie = $let_serie;
            $info->let_numero = $let_numero;
            if ($forma_pago == 1){
               $this->pago_caja($info);
            }
     ...
     public function pago_caja($info){
            $fecha = date("Y-m-d");
            ...
            if ($info->caja != "" && $info->caja != NULL){
                ...
                $filter->CAJAMOV_MovDinero = 1; # (V:1) = INGRESO | (C:2) = EGRESO
                $filter->FORPAP_Codigo = $info->FORPAP_Codigo; #$comprobante->FORPAP_Codigo; # LAS CUOTAS SE PAGAN EN EFECTIVO
                $filter->CAJAMOV_FechaRecep = $fecha;
                $filter->CAJAMOV_Justificacion = "PAGO DE LETRA: ".$info->let_serie . " - " . $info->let_numero;
                $filter->CAJAMOV_Observacion = "PAGO DE LETRA: ".$info->let_serie . " - " . $info->let_numero." / ".$info->observacion;
    ...
    public function letra_pdf($letra, $flagImage = 1){
     ...
     public function guardar_pago_letra($info){
            $fecha = date("Y-m-d");
            $fechaRegistro = date("Y-m-d h:i:s");

            $comprobante = $this->comprobante_model->obtener_comprobante($info->comprobante);
            $tipo_cuenta = ($comprobante[0]->CPC_TipoOperacion == "V") ? 1 : 2; # 1 COBRAR 2 PAGAR
            ...
            $filter->CLIP_Codigo = $comprobante[0]->CLIP_Codigo;
            $filter->PROVP_Codigo = $comprobante[0]->PROVP_Codigo;
            $filter->PAGC_TDC = $this->tipocambio_model->getCambio($comprobante[0]->MONED_Codigo, $fecha);
            $filter->PAGC_FormaPago = $info->FORPAP_Codigo;; # POR AHORA EFECTIVO
            ...
            $cuentaInfo = $this->cuentas_model->getCuentaComprobante($comprobante[0]->CPP_Codigo);
            ...
            if ($cuentaInfo != NULL){
                ...
                $filter->MONED_Codigo   = $comprobante[0]->MONED_Codigo;
