...
//                }
    ...
    public function comprobantes($tipo_oper = '', $tipo_docu = '', $j = '0')
    {
        ...
        $data['action'] = 'index.php/ventas/importacion/comprobantes/' . $tipo_oper . '/' . $tipo_docu;


        $conf['base_url'] = site_url('ventas/importacion/comprobantes/' . $tipo_oper . '/' . $tipo_docu);
        ...
        if (count($listado_comprobantes) > 0) {
            foreach ($listado_comprobantes as $indice => $valor) {
                ...
                if($tipo_docu=='N' && $tipo_oper=='V'){
                    ...
                    if ($tipo_oper=='C') {
                        $ver = "<a href='javascript:;' onclick='comprobante_ver_pdf_conmenbrete(". $codigo .",".$ConversorDeNumero.",0,".$tipo_oper2.")' target='_parent'><img src='" . base_url() . "images/icono_imprimir.png' width='16' height='16' border='0' title='Imprimir COMPRA'></a>";
                    }
                    if($tipo_oper=='V'){
                        $ver = "<a href='javascript:;' onclick='comprobante_ver_pdf_conmenbrete(". $codigo .",".$ConversorDeNumero.",0,".$tipo_oper2.")' target='_parent'><img src='" . base_url() . "images/icono_imprimir.png' width='16' height='16' border='0' title='Imprimir VENTA'></a>";
    ...
    public function comprobante_insertar(){
        ...
        if ($this->input->post('tipo_oper') == 'C' && $this->input->post('moneda') > 2 && !preg_match(/^[0-9\.]+$/, $this->input->post('tdcOpcional')))
            exit('{"result":"error", "campo":"tdcOpcional"}');
...
//         }
        ...
        $filter->IMPOR_TDC = $this->input->post('moneda') > 2 ? $this->input->post('tdcOpcional') : $this->input->post('tdc');

        if ($this->input->post('moneda') > 2) $filter->IMPOR_TDC_opcional = $this->input->post('tdc');
...
//Datos cabecera de la guiasa.
    ...
    public function comprobante_ver()
    {
        $this->load->library('layout', 'layout');
        $comp_confi = $this->companiaconfiguracion_model->obtener($this->somevar['compania']);
