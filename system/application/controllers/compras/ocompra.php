...
class Ocompra extends Controller{
    ...
    public function ocompras($j = '0', $tipo_oper = 'C', $eval = '0'){
        ...
        $data['titulo_tabla'] = "RELACIÓN DE " . ($tipo_oper == 'V' ? 'PEDIDOS DE VENTA' : 'ORDENES DE COMPRA');
        $data['titulo_busqueda'] = "BUSCAR " . ($tipo_oper == 'V' ? ' PEDIDOS DE VENTA' : 'ORDENES DE COMPRA');
    ...
    public function datatable_ocompra($tipo_oper = 'V'){
        ...
        if (count($listado_ocompras) > 0) {
            foreach ($listado_ocompras as $indice => $valor) {
                ...
                $enviarcorreo =  "<a onclick='open_mail($codigo);' href='javascript:;' class='enviarcorreo'><img src='" . base_url() . "images/send.png' width='16' height='16' border='0' title='Enviar Cotizacion via correo'></a>";
                ...
                $pdfImprimir = "<a href='".base_url()."index.php/compras/ocompra/ocompra_ver_pdf_conmenbrete/$codigo/0' data-fancybox data-type='iframe'><img src='" . base_url() . "images/icono_imprimir.png' width='16' height='16' border='0' title='Imprimir'></a>";
                $pdfImprimir2 = "<a href='".base_url()."index.php/compras/ocompra/ocompra_ver_pdf_conmenbrete/$codigo/1' data-fancybox data-type='iframe'><img src='" . base_url() . "images/pdf.png' width='16' height='16' border='0' title='Ver PDF'></a>";
                ...
                if ( $valor->OCOMC_FlagEstado == "1"){
                    ...
                    $canjeComprobante = ($comprobante != NULL) ? "<a href='".base_url()."index.php/ventas/comprobante/comprobante_ver_pdf/$comprobante/a4' data-fancybox data-type='iframe'> <span style='font-weight: bold; font-size: 7pt; color:green'>►$comprobante_serieNumero</span> </a>" : "";
                }
                ...
                $lista[] = array(
                                    ...
                                    10 => $excel,
                                    11 => $enviarcorreo,
                                    12 => "<div align='left' class='cResult_$item'>$canjeGuia</div> <div align='left'><span class='icon-loading loading_c_$item'></span></div>"
                                );
                $item++;
            }
        }
    ...
    public function seguimiento_ocompras($j = '0', $tipo_oper = 'C', $eval = '0'){
        ...
        $data['titulo_tabla'] = "SEGUIMIENTO PEDIDO DE " . ($tipo_oper == 'V' ? 'VENTA' : 'COMPRA');
        $data['titulo_busqueda'] = "BUSCAR SEGUIMIENTO DE PEDIDO DE" . ($tipo_oper == 'V' ? 'VENTA' : 'COMPRA');
        $data['tipo_oper'] = $tipo_oper;
        $data['id_documento'] = ($tipo_oper == C) ? 3 : 18;
        $data['cboVendedor'] = $this->lib_props->listarVendedores();
        $data['oculto'] = form_hidden(array('base_url' => base_url(), 'tipo_oper' => $tipo_oper));
       ...
       $columnas = array(
        ...
        if (count($listado_ocompras) > 0) {
            foreach ($listado_ocompras as $indice => $valor) {
                ...
                $pedido = $valor->PEDIP_Codigo;
                $numero = $valor->OCOMC_Serie."-".$valor->OCOMC_Numero;
                $cliente = $valor->CLIP_Codigo;
                $proveedor = $valor->PROVP_Codigo;
                $ccosto = $valor->CENCOSP_Codigo;
                $total = $valor->OCOMC_total;
                ...
                if ($cantidad_entregada == 0) {
                    $url_img = "images/ninguno.png";
                    $msguiain = "<div style='background-color: rgba(194,0,0,1); padding: 0.5em 1em; text-align: center; color: rgba(255,255,255,1)'>Pend.</div>";
                }
                if ($cantidad_entregada > 0) {
                    $url_img = "images/proceso.png";
                    $msguiain = "<div style='background-color: rgba(200,200,0,1); padding: 0.5em 1em; text-align: center; color: rgba(255,255,255,1)'>Carg.</div>";
                }
                if ($cantidad_entregada == $cantidad_total) {
                    $url_img = "images/entregado.png";
                    $msguiain = "<div style='background-color: rgba(3,170,3,1); padding: 0.5em 1em; text-align: center; color: rgba(255,255,255,1)'>Term.</div>";
                    $edit = false;
                }
    ...
    public function nueva_ocompra($tipo_oper = 'C'){
        ...
        $data['titulo'] = "REGISTRAR " . ($tipo_oper == 'V' ? 'PEDIDO DE VENTA' : 'ORDENES DE COMPRA');
        $data['tipo_oper'] = $tipo_oper;
    ...
    public function insertar_ocompra(){
        ...
        #datos correlativo
        $datos_configuracion = $this->configuracion_model->obtener_numero_documento($this->somevar['compania'], $id_documento = ($tipo_oper == 'C' ? '3' : '18'));
    ...
    public function insertar_comprobante($codigoOC){
        ...
        $success = array(
                            "result" => "success",
                            "pdf" => "<a href='".base_url()."index.php/ventas/comprobante/comprobante_ver_pdf/$comprobante/a4' data-fancybox data-type='iframe'> <span style='font-weight: bold; font-size: 7pt; color:green'>► $cSerie-".$this->lib_props->getOrderNumeroSerie($cNumero)."</span> </a>"
                        );
