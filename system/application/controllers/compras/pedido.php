...
class Pedido extends Controller{
    ...
    public function ocompras($j = '0', $tipo_oper = 'C', $eval = '0'){
        ...
        $data['registros'] = $this->pedido_model->total_ocompra($tipo_oper);
        $conf['base_url'] = site_url('compras/ocompra/pedido/0/' . $tipo_oper . '/0/');
        $data['titulo_tabla'] = "RELACIÓN DE COTIZACIÓN DE " . ($tipo_oper == 'V' ? 'VENTA' : 'COMPRA');
        $data['titulo_busqueda'] = "BUSCAR COTIZACIÓN DE " . ($tipo_oper == 'V' ? 'VENTA' : 'COMPRA');
        $data['tipo_oper'] = $tipo_oper;
        $data['id_documento'] = ($tipo_oper == C) ? 13 : 21;
    ...
    public function datatable_ocompra($tipo_oper = 'V'){
        ...
        $listado_ocompras = $this->pedido_model->getOcompra($filter);
        $lista = array();
        if (count($listado_ocompras) > 0) {
            foreach ($listado_ocompras as $indice => $valor) {
                ...
                //var_dump($cotizacion);exit();
                $comprobante_serieNumero="";
                    
                if ($cotizacion!=null || $cotizacion!="") {
                    $datos_ocompra = $this->ocompra_model->obtener_ocompra($cotizacion);
                    $numero_ocompra = $datos_ocompra[0]->OCOMC_Numero;
                    $serie_ocompra = $datos_ocompra[0]->OCOMC_Serie;
                    $comprobante_serieNumero = $serie_ocompra."-".$numero_ocompra;
                ...
                //var_dump($comprobante_serieNumero);exit();
                ...
                if ($cliente != '' && $cliente != '0') {
                    ...
                    $ruc = ($datos_cliente->ruc != '' && $datos_cliente->ruc != 0) ? $datos_cliente->ruc : $datos_cliente->dni;
                ...
                /*if ($cliente != '' && $cliente != '0') {
                    $nombre = "<div class='tip'> $nombre <span class='msg'>VENDEDOR: $valor->razon_social_cliente $valor->vendedor </span> </div>";
                }*/
                ...
                if ($cantidad_entregada == 0) {
                    ...
                    $msguiain = "<span style='color: #8c1a16; padding: 2px 15px 2px 15px; font-weight: bolder; font-size: 11px' title='Pendiente' >Pend.</span>";
                }
                if ($cantidad_entregada > 0) {
                    $url_img = "images/proceso.png";
                    $msguiain = "<span style='color: #8c8b02; padding: 2px 15px 2px 15px; font-weight: bolder; font-size: 11px' title='Cargando' >Carg.</span>";
                }
                if ($cantidad_entregada >= $cantidad_total) {
                    $url_img = "images/entregado.png";
                    $msguiain = "<span style='color: green; padding: 2px 15px 2px 15px; font-weight: bolder; font-size: 11px' title='Terminado' >Term.</span>";
                    $edit = false;

                    if ($estado == "2"){
                        $filterU = new stdClass();
                        $filterU->OCOMC_FlagEstado = 1;
                        $this->pedido_model->modificar_ocompra($codigo, $filterU);
                    }
                }*/
                ...
                $enviarcorreo =  "<a onclick='open_mail($codigo);' href='javascript:;' class='enviarcorreo'><img src='" . base_url() . "images/send.png' width='16' height='16' border='0' title='Enviar Pedido via correo'></a>";
                
                
                if ($edit == false)
                    $editar = '';
                
                $pdfImprimir = "<a href='".base_url()."index.php/compras/pedido/pedido_ver_pdf_conmenbrete/$codigo/0' data-fancybox data-type='iframe'><img src='" . base_url() . "images/icono_imprimir.png' width='16' height='16' border='0' title='Imprimir'></a>";
                $pdfImprimir2 = "<a href='".base_url()."index.php/compras/pedido/pedido_ver_pdf_conmenbrete/$codigo/1' data-fancybox data-type='iframe'><img src='" . base_url() . "images/pdf.png' width='16' height='16' border='0' title='Ver PDF'></a>";
                ...
                if ( $valor->OCOMC_FlagEstado == "1"){
                        ...
                        if ($tipoComprobante == 'D'){
                            $canjeGuia = "";
                             $canjeComprobante = ($cotizacion != NULL) ? "<a href='".base_url()."index.php/compras/ocompra/ocompra_ver_pdf_conmenbrete/$cotizacion/1' data-fancybox data-type='iframe'> <span style='font-weight: bold; font-size: 7pt; color:green'>►$comprobante_serieNumero</span> </a>" : "<a href='javascript:;' onclick='canjeToComprobante($codigo, $item)' title='Convertir Cotizacion' style='font-weight: bold; font-size: 7pt;'>► $tComprobante</a>";
                        }else{
                    ...
                    $canjeComprobante = ($cotizacion != NULL) ? "<a href='".base_url()."index.php/ventas/comprobante/comprobante_ver_pdf/$cotizacion/a4' data-fancybox data-type='iframe'> <span style='font-weight: bold; font-size: 7pt; color:green'>►$comprobante_serieNumero</span> </a>" : "";
                }
                ...
                $lista[] = array(
                                    ...
                                    $pos++ => $excel,
                                    $pos++ => $enviarcorreo,
                                    $pos++ => "<div align='left' class='cResult_$item'>$canjeComprobante</div> <div align='left'><span class='icon-loading loading_c_$item'></span></div>"
                                );
                $item++;
            }
        }
        ...
        $json = array(
                            ...
                            "recordsTotal"    => count($this->pedido_model->getOcompra($filterAll)),
                            "recordsFiltered" => intval( count($this->pedido_model->getOcompra($filter)) ),
                            "data"            => $lista
                    );
    ...
    public function datatable_seguimientoOcompra($tipo_oper='C'){
        ...
        if (count($listado_ocompras) > 0) {
            foreach ($listado_ocompras as $indice => $valor) {
                ...
                $ver = "<a href='javascript:;' onclick='pedido_ver_pdf(" . $codigo . ")'><img src='" . base_url() . "images/icono_imprimir.png' width='16' height='16' border='0' title='Imprimir'></a>";
                $ver2 = "<a href='javascript:;' onclick='pedido_ver_pdf_conmenbrete(" . $codigo . ")'><img src='" . base_url() . "images/pdf.png' width='16' height='16' border='0' title='Ver PDF'></a>";
                $eliminar = "<a href='javascript:;' onclick='eliminar_ocompra(" . $codigo . ")'><img src='" . base_url() . "images/eliminar.png' width='16' height='16' border='0' title='Eliminar'></a>";
                $lista[] = array(
                    $fecha, 
                    $numero, 
    ...
    public function nueva_ocompra($tipo_oper = 'C'){
        ...
        $data['url_action'] = base_url() . "index.php/compras/pedido/insertar_ocompra";
        $data['titulo'] = "REGISTRAR COTIZACIÓN DE " . ($tipo_oper == 'V' ? 'VENTA' : 'COMPRA');
        $data['tipo_oper'] = $tipo_oper;
        ...
        $cofiguracion_datos = $this->configuracion_model->obtener_numero_documento($compania, $tipo_oper == 'C' ? '13' : '21');
    ...
    public function listar_contactos_empresa($codigo = NULL){
        ...
        //var_dump($codigo);
        $compania = $this->somevar['compania'];
        $datosCliente=$this->pedido_model->listar_clientes_cotizacion($codigo);
        $result = array();
    ...
    public function insertar_ocompra(){
        ...
        #datos correlativo
        $datos_configuracion = $this->configuracion_model->obtener_numero_documento($this->somevar['compania'], $id_documento = ($tipo_oper == 'C' ? '13' : '21'));
        $numero = $datos_configuracion[0]->CONFIC_Numero + 1;

        $filter->OCOMC_Numero = $numero;

        if($id_documento == 13) $filter->OCOMC_FlagBS = '';

        $ocompra = $this->pedido_model->insertar_ocompra($filter);
        ...
        if (is_array($prodcodigo)) {
                ...
                if($accion != "e" && $accion != "EE"){
                    ...
                    $this->pedidodetalle_model->insertar($filter);
                     ...
                     if($tipo_oper == 'C'  && !is_null($codoventa) && $codoventa != ''  ){
                            //$this->pedido_model->modificar_pendienteoventa($codoventa,$filter->PROD_Codigo,$filter->OCOMDEC_Cantidad);
                            $this->pedido_model->modificar_pendiente_cantidad_id_detalle_venta($codoventa, -$filter->OCOMDEC_Cantidad);
                        }
            ...
            foreach ($oventas as  $codoventa) {
                ...
                if($tipo_oper == 'C'  && !is_null($codoventa) && $codoventa != '' ){
                                $detalle_ocompra=$this->pedido_model->esta_importado($codoventa);
                               // $cantidadp=$detalle_ocompra->OCOMDEC_Pendiente;
                                    if ($detalle_ocompra) {
                                         $this->pedido_model->modificar_flagTerminado_oventa($codoventa, "1");
                                    }
        ...
        //if ( $this->input->post('estado') == "1" ){
            //$this->lib_props->sendMail(39, $ocompra); # MENU 60 = "Seguimiento de Cotizaciones",
        //}
    ...
    public function ver_detalle_ocompra($codigo, $tipo_oper = 'C')
    {
        ...
        $data['contiene_igv'] = (($comp_confi[0]->COMPCONFIC_PrecioContieneIgv == '1') ? true : false);
        $oculto = form_hidden(array('accion' => $accion, 'codigo' => $codigo, 'empresa' => $empresa, 'persona' => $persona, 'modo' => $modo, 'base_url' => base_url(), 'tipo_oper' => $tipo_oper, 'contiene_igv' => ($data['contiene_igv'] == true ? '1' : '0')));
        $data['titulo'] = "ESTADO DE PEDIDO DE " . ($tipo_oper == 'V' ? 'VENTA' : 'COMPRA');
        $data['formulario'] = "frmOrdenCompra";
        $data['oculto'] = $oculto;
        $data['url_action'] = base_url() . "index.php/compras/pedido/actualizarEvaluacion";
        $atributos = array('width' => 700, 'height' => 450, 'scrollbars' => 'yes', 'status' => 'yes', 'resizable' => 'yes', 'screenx' => '0', 'screeny' => '0');
        $contenido = "<img height='16' width='16' src='" . base_url() . "images/ver.png' title='Buscar' border='0'>";
        $data['vercliente'] = anchor_popup('ventas/cliente/ventana_busqueda_cliente', $contenido, $atributos, 'linkVerCliente');
        $data['verproveedor'] = anchor_popup('compras/proveedor/ventana_busqueda_proveedor', $contenido, $atributos, 'linkVerProveedor');
        ...
        $data['detalle_ocompra'] = $detalle_ocompra;
        $this->layout->view('compras/pedido_detalle', $data);
    }
    ...
    public function actualizarEvaluacion(){
        if ( $this->input->post('process') == 0 ){ // Si no ha sido evaluado antes
            ...
            if ($estado != '' && $codigo != ''){
                if ($estado != "0"){
                    ...
                    $this->pedido_model->modificar_ocompra($codigo, $filter);
        ...
        $json = array("redirect" => base_url()."index.php/compras/pedido/seguimiento_ocompras/0/V/1", "msg" => $result);
        echo json_encode($json);
        #redirect(base_url()."index.php/compras/pedido/seguimiento_ocompras/0/V/1");
    }

    public function insertar_guiarem($codigoOC){

        $datos_ocompra = $this->pedido_model->obtener_ocompra($codigoOC);
        ...
        ##########################################
            ...
            $filter->GUIAREMC_UbigeoLlegada = $ubigeo_destino;
        ...
        $detalle = $this->pedido_model->obtener_detalle_ocompra($codigoOC);
        $detalle_ocompra = array();
        if (count($detalle) > 0) {
            foreach ($detalle as $indice => $valor) {
                $detocompra = $valor->OCOMDEP_Codigo;
        ...
        $success = array(
                            "result" => "success",
                            "pdf" => "<a href='javascript:;' onclick='comprobante_ver_pdf_conmenbrete($guiarem_id, 10, \"a4\")'> <span style='font-weight: bold; font-size: 7pt; color:green'>► $serie-$numero</span> </a>"
                        );
    ...
    public function insertar_comprobante($codigoOC){
        ...
        $datos_pedido = $this->pedido_model->obtener_ocompra($codigoOC);
        ...
        ##########################################################################
            ...
            $tipo_docu = $datos_pedido[0]->CPC_TipoDocumento;
            ...
            $direccion = "";
            if ($tipo_oper == 'V'){
                if ($datos_pedido[0]->OCOMC_FactDireccion != "")
                    $direccion = $datos_pedido[0]->OCOMC_FactDireccion;
                else{
                    $datos_cliente = $this->cliente_model->obtener($cliente);
        ...
        ##########################################################################
            $filter = new stdClass();
            $filter->OCOMC_TipoOperacion = $tipo_oper;
            $filter->OCOMC_CodigoUsuario = $datos_pedido[0]->OCOMC_CodigoUsuario;
            
            $filter->OCOMC_Entrega = $datos_pedido[0]->OCOMC_Entrega;

            if ($tipo_oper == 'V') {
                $filter->CLIP_Codigo = $datos_pedido[0]->CLIP_Codigo;
            } else {
                $filter->PROVP_Codigo = $datos_pedido[0]->PROVP_Codigo;
            }

            $filter->COTIP_Codigo = $codigoOC;#AQUI SE PUEDE AGREGAR EL PEDIDO ASOCIADO
            $filter->OCOMP_CodigoVenta = null;
            $filter->PROYP_Codigo = null;
            $filter->PRESUP_Codigo = NULL; #AQUI SE PUEDE AGREGAR EL PEDIDO ASOCIADO
            $filter->MONED_Codigo = $datos_pedido[0]->MONED_Codigo;
            $filter->OCOMC_descuento100 = $datos_pedido[0]->OCOMC_descuento100;
            $filter->OCOMC_PersonaAutorizada = $datos_pedido[0]->OCOMC_PersonaAutorizada;
            $filter->OCOMC_MiPersonal = $datos_pedido[0]->OCOMC_MiPersonal;
            $filter->OCOMC_Personal = $datos_pedido[0]->OCOMC_Personal;
            $filter->OCOMC_igv100 = $datos_pedido[0]->OCOMC_igv100;
            $filter->OCOMC_subtotal = $datos_pedido[0]->OCOMC_subtotal;
            $filter->OCOMC_descuento = $datos_pedido[0]->OCOMC_descuento;
            $filter->OCOMC_igv = $datos_pedido[0]->OCOMC_igv;
            $filter->OCOMC_total = $datos_pedido[0]->OCOMC_total;
            $filter->OCOMC_percepcion = $datos_pedido[0]->OCOMC_percepcion;
            $filter->CENCOSP_Codigo = $datos_pedido[0]->CENCOSP_Codigo;
            $filter->OCOMC_Observacion = $datos_pedido[0]->OCOMC_Observacion;
            $filter->ALMAP_Codigo = $datos_pedido[0]->ALMAP_Codigo;
            $filter->FORPAP_Codigo = $datos_pedido[0]->FORPAP_Codigo;
            $filter->OCOMC_EnvioDireccion = $datos_pedido[0]->OCOMC_EnvioDireccion;
            $filter->OCOMC_FactDireccion = $datos_pedido[0]->OCOMC_FactDireccion;
            $filter->PROYP_Codigo = $datos_pedido[0]->PROYP_Codigo;
            $filter->OCOMP_TDC = $datos_pedido[0]->OCOMP_TDC;
            $filter->OCOMP_TDC_opcional = $datos_pedido[0]->OCOMP_TDC_opcional;
            $filter->OCOMC_Fecha = $fecha;
            $filter->OCOMC_FechaEntrega = $datos_pedido[0]->OCOMC_FechaEntrega;
            $filter->OCOMC_CtaCteSoles = $datos_pedido[0]->OCOMC_CtaCteSoles;
            $filter->OCOMC_CtaCteDolares = $datos_pedido[0]->OCOMC_CtaCteDolares;
            $filter->OCOMC_FlagEstado = $datos_pedido[0]->OCOMC_FlagEstado;
            $filter->CPC_TipoDocumento = 'F';
            #datos correlativo
            $id_documento = ($tipo_oper == 'C' ? '3' : '18');
            $datos_configuracion = $this->configuracion_model->obtener_numero_documento($compania, $id_documento);
            $numero = $datos_configuracion[0]->CONFIC_Numero + 1;
            $filter->OCOMC_Numero = $numero;
            if($tipo_oper == 'C') $filter->OCOMC_FlagBS = '';
            $serie = $datos_configuracion[0]->CONFIC_Serie;
            $filter->OCOMC_Serie = $serie;        
        ...
        if (count($detalle) > 0) {
            foreach ($detalle as $indice => $valor) {
                ...
                $filterDet->OCOMP_Codigo            = $ocompra;
                $filterDet->PROD_Codigo             = $valor->PROD_Codigo;
                $filterDet->OCOMDEC_Cantidad        = $valor->OCOMDEC_Cantidad;
                $filterDet->OCOMDEC_Pendiente       = $valor->OCOMDEC_Pendiente;
                $filterDet->UNDMED_Codigo           = $valor->UNDMED_Codigo;
                $filterDet->AFECT_Codigo            = $valor->AFECT_Codigo;
                $filterDet->OCOMDEC_Pendiente_pago  = $valor->OCOMDEC_Pendiente_pago;
                $filterDet->OCOMDEC_Descuento100    = $valor->OCOMDEC_Descuento100; 
                $filterDet->OCOMDEC_Igv100          = $valor->OCOMDEC_Igv100;
                $filterDet->OCOMDEC_Pu              = $valor->OCOMDEC_Pu; 
                $filterDet->OCOMDEC_Subtotal        = $valor->OCOMDEC_Subtotal;
                $filterDet->OCOMDEC_Descuento       = $valor->OCOMDEC_Descuento;
                $filterDet->OCOMDEC_Descuento2      = 0;
                $filterDet->OCOMDEC_Igv             = $valor->OCOMDEC_Igv;
                $filterDet->OCOMDEC_Total           = $valor->OCOMDEC_Total;
                $filterDet->OCOMDEC_Pu_ConIgv       = $valor->OCOMDEC_Pu_ConIgv;
                $filterDet->OCOMDEC_Costo           = $valor->OCOMDEC_Costo;
                $filterDet->OCOMDEC_Descripcion     = $valor->OCOMDEC_Descripcion;
                $filterDet->OCOMDEC_GenInd          = $valor->OCOMDEC_GenInd;
                $filterDet->OCOMDEC_Observacion     = $valor->OCOMDEC_Observacion;
                $filterDet->OCOMDEC_flete           = $valor->OCOMDEC_flete;
                $filterDet->OCOMP_Codigo_venta      = $valor->OCOMP_Codigo_venta;

                $this->ocompradetalle_model->insertar($filterDet);
            }
        }

        ### ASOCIAMOS LA COTIZACION GENERADA AL PEDIDO
        $filter = new stdClass();
        if ($tipo_oper == 'V') {
            $filterMod->COTIP_Codigo = $ocompra;
        } else {
            $filterMod->COTIP_Codigo = $ocompra;
        }
        $this->pedido_model->modificar_ocompra($codigoOC, $filterMod);
        if ($tipo_oper == 'V'){
            $this->lib_props->sendMail(39, $ocompra, NULL, "UN PRESUPUESTO HA SIDO APROBADO PARA SER DESPACHADO. <b>Serie: $serie-$numero</b>"); # MENU 61 = Órdenes de pedidos
        }
        $success = array(
                            "result" => "success",
                            "pdf" => "<a href='".base_url()."index.php/compras/ocompra/ocompra_ver_pdf_conmenbrete/$ocompra/1' data-fancybox data-type='iframe'><span style='font-weight: bold; font-size: 7pt; color:green'>► $serie-".$this->lib_props->getOrderNumeroSerie($numero)."</span> </a>"
                        );
            ...
            foreach ($detacodi as $indice => $valor) {
                ...
                if ($detalle_accion == 'n') {
                    ...
                    if($tipo_oper == 'C'  && !is_null($codovente) && $codovente != ''  ){
                        $this->pedido_model->modificar_pendiente_by_id_detalle_venta($codovente, $filter->OCOMDEC_Cantidad);
                    }
                }
                else
                    if ($detalle_accion == 'm') {
                        ...
                        if($tipo_oper == 'V'){
                            if($cantreal[$indice] == $prodcantidad[$indice] && $prodcantidad[$indice] == $pendiente[$indice]){
                                ...
                                if(isset($filter->OCOMDEC_Pendiente_pago)) {
                                    ...
                                    $this->pedido_model->modificar_pendiente_pago_by_id_detalle_venta($valor, - $cantmed);
                                }
                            }
                        ...
                        if($tipo_oper == 'C' && !is_null($codovente) && $codovente != ''){
                            $this->pedido_model->modificar_pendiente_by_id_detalle_venta($codovente, - $pendientef);
                        }
                }
                else
                    if ($detalle_accion == 'e') {
    ...
    {
        $codigo = $this->input->post('codigo');
        $this->pedido_model->eliminar($codigo);
    }
    ...
    public function evaluar_ocompra()
    {
        ...
        if (is_array($checkO))
            $this->pedido_model->evaluar_ocompra($flag, $checkO);
    ...
    public function pedido_ver_pdf($codigo)
    {
        switch (FORMATO_IMPRESION) {
            case 1: //Formato para ferresat
                $this->pedido_ver_pdf_formato1($codigo);
                break;
            case 2:  //Formato para jimmyplat
                $this->pedido_ver_pdf_formato2($codigo);
                break;
            case 3:  //Formato para instrumentos y systemas
                $this->pedido_ver_pdf_formato3($codigo);
                break;
            case 4:  //Formato para ferremax
                $this->pedido_ver_pdf_formato4($codigo);
                break;
            default:
                $this->pedido_ver_pdf_formato1($codigo);
                break;
    ...
    public function pedido_ver_pdf_conmenbrete($codigo, $flagPdf = 1)
    {
        switch (FORMATO_IMPRESION) {
            case 1: //Formato para ferresat
                #$this->pedido_ver_pdf_conmenbrete_formato1($codigo, $flagPdf);
                $this->pedido_ver_pdf_a4($codigo, $flagPdf);
                break;
            case 2:  //Formato para jimmyplat
                $this->pedido_ver_pdf_conmenbrete_formato2($codigo);
                break;
            case 3:  //Formato para instrumentos y systemas
                $this->pedido_ver_pdf_conmenbrete_formato3($codigo);
                break;
            case 4:  //Formato para ferremax
                $this->pedido_ver_pdf_conmenbrete_formato4($codigo);
                break;
            default:
                $this->pedido_ver_pdf_conmenbrete_formato1($codigo, $flagPdf);
                break;
    ...
    public function pedido_ver_pdf_formato1($codigo)
    {
        ...
        /* Datos principales */
        $datos_ocompra = $this->pedido_model->obtener_ocompra($codigo);
        $datos_detalle_ocompra = $this->pedido_model->obtener_detalle_ocompra($codigo);
        $tipo_oper = $datos_ocompra[0]->OCOMC_TipoOperacion;
        $cotizacion = $datos_ocompra[0]->COTIP_Codigo;
        $pedido = $datos_ocompra[0]->PEDIP_Codigo;
        $numero = $datos_ocompra[0]->OCOMC_Numero;
    ...
    public function pedido_ver_pdf_formato2($codigo)
    {
        //$this->load->library('cezpdf');
        //$this->load->helper('pdf_helper');
        $this->load->model('almacen/almacen_model');
    ...
    public function pedido_ver_pdf_formato3($codigo)
    {
        //$this->load->library('cezpdf');
        //$this->load->helper('pdf_helper');
        $this->load->model('almacen/almacen_model');
    ...
    public function pedido_ver_pdf_formato4($codigo)
    {
        //$this->load->library('cezpdf');
        //$this->load->helper('pdf_helper');
        $this->load->model('almacen/almacen_model');
    ...
    public function pedido_ver_pdf_conmenbrete_formato1($codigo, $flagPdf)
    {
        $this->load->model('almacen/almacen_model');
    ...
    public function pedido_ver_pdf_a4($codigo, $flagPdf = 1, $enviarcorreo = false){
        $this->lib_props->pedido_pdf($codigo, $flagPdf, $enviarcorreo);
        return NULL;
    }

    public function pedido_ver_pdf_rango($inicio = 1, $fin = 20, $tipo_oper = "V", $flagPdf = 0, $enviarcorreo = false){
    ...
    public function pedido_ver_pdf_conmenbrete_formato2($codigo)
    {
        //$this->load->library('cezpdf');
        //$this->load->helper('pdf_helper');
        $this->load->model('almacen/almacen_model');
    ...
    public function pedido_ver_pdf_conmenbrete_formato3($codigo)
    {
        //$this->load->library('cezpdf');
        //$this->load->helper('pdf_helper');
        $this->load->model('almacen/almacen_model');
    ...
    public function pedido_ver_pdf_conmenbrete_formato4($codigo)
    {
        //$this->load->library('cezpdf');
        //$this->load->helper('pdf_helper');
        $this->load->model('almacen/almacen_model');
    ...
    public function ventana_muestra_ocompra($tipo_oper, $codigo = '', $formato = 'SELECT_ITEM', $docu_orig = '', $almacen = "", $comprobante = '', $ventana = ''){
        ...
        $lista_comprobante = $this->pedido_model->buscar_ocompra_asoc($tipo_oper, $comprobante, $filter);
        ...
        /*foreach ($lista_comprobante as $indice => $value) {
            ...
            if ($formato == 'SELECT_HEADER') {
                if ($ventana != 'OC') {
                    //$select = "<a href='" . base_url() . "index.php/compras/pedido/comprobante_nueva_ocompra/" . $value->OCOMP_Codigo . "/" . $tipo_oper . "' id='linkVerOrdenCompra' ><img src='" . base_url() . "images/ir.png' width='16' height='16' border='0' title='Seleccionar O. compra'></a>";
    ...
    public function datatable_ventana_ocompra(){
        ...
        $lista_comprobante = $this->pedido_model->getOcompraAsoc($filter);
        ...
        $json = array(
                            "draw"            => intval( $this->input->post('draw') ),
                            "recordsTotal"    => count($this->pedido_model->getOcompraAsoc($filterAll)),
                            "recordsFiltered" => intval( count($this->pedido_model->getOcompraAsoc($filter)) ),
                            "data"            => $lista
                    );
    ...
    public function ventana_muestra_oventa($tipo_oper = 'V',$formato = 'SELECT_HEADER', $docu_orig = "", $comprobante = '', $ventana = 'OC')
    {
        ...
        $lista_comprobante = $this->pedido_model->buscar_oventa_asoc('V', $comprobante);
    ...
    public function ventana_muestra_ocompra_importacion($tipo_oper, $codigo = '', $formato = 'SELECT_ITEM', $tipo_doc = '', $almacen = "", $comprobante = '')
    {
        ...
        $lista_comprobante = $this->pedido_model->buscar_ocompra_importacion($tipo_oper, $comprobante, $filter);
    ...
    public function obtener_detalles_ocompras()
    {
        $detalle = array();
        $codigo_moneda = 0;
        foreach ($this->pedido_model->listarByIds($this->input->post("compras")) as $articulo) {
    ...
    {
        $datos_detalle_ocompra = $this->pedido_model->obtener_detalle_ocompra($ocompra);
        $listado = array();
        if (count($datos_detalle_ocompra) > 0) {
            foreach ($datos_detalle_ocompra as $indice => $valor) {
                $detocompra = $valor->OCOMDEP_Codigo;
                ...
                $datos_ocompra = $this->pedido_model->obtener_ocompra($ocompra);
                $proveedor = $datos_ocompra[0]->PROVP_Codigo;
                $almacen = $datos_ocompra[0]->ALMAP_Codigo;
                $formapago = $datos_ocompra[0]->FORPAP_Codigo;
    ...
    {
        $datos_detalle_ocompra = $this->pedido_model->obtener_detalle_ocompra2($ocompra);
        $listado = array();
        if (count($datos_detalle_ocompra) > 0) {
            foreach ($datos_detalle_ocompra as $indice => $valor) {
                $detocompra = $valor->OCOMDEP_Codigo;
        ...
        public function obtener_detalle_ocompra2($ocompra, $tipo = NULL)
        {
            ...
            if (count($datos_detalle_ocompra) > 0) {
                foreach ($datos_detalle_ocompra as $indice => $valor) {
                    ...
                    if(!isset($ocompras_list[$ocompra])) $ocompras_list[$ocompra] = $this->pedido_model->obtener_ocompra($ocompra);
                    ...
                    if(true) {
                        if(!isset($prices_list[$valor->PROD_Codigo])) $prices_list[$valor->PROD_Codigo] = $this->pedido_model->get_last_price_by_id_product($valor->PROD_Codigo);
    ...
    */
    ...
    public function obtener_detalle_ocompra_for_comprobante($ocompra, $tipo = NULL)
    {
        ...
        if (count($datos_detalle_ocompra) > 0) {
            foreach ($datos_detalle_ocompra as $indice => $valor) {
                ...
                $referencia_oventa = $this->pedido_model->obtener_referencia_compra_by_id_detalle_venta($valor->OCOMP_Codigo_venta);
    ...
    /* Combos */
     public function calcula_ocantidad_pendiente($ocodigo,$prod,$cant){
        $cant=$cant;
        $verificar_cantidad=$this->pedido_model->calcula_ocantidad_pendiente($ocodigo,$prod);
    ...
    public function calcula_ocantidad_pendiente_by_id_detalle($id_detalle, $cant, $current_cant){
        $cant=$cant;
        $verificar_cantidad=$this->pedido_model->calcula_ocantidad_pendiente_by_id_detalle($id_detalle);
        $cantidad=$verificar_cantidad->OCOMDEC_Cantidad;  //4
        $pendiente=$verificar_cantidad->OCOMDEC_Pendiente;  //2
    ...
    public function cantidad_oregistrada($oventa,$prod,$cant,$pend){
        $cant=$cant;
        $pend=$pend;
        $verificar_cantidad=$this->pedido_model->verificar_ocantidad($oventa,$prod);
        $cantidad=$verificar_cantidad->OCOMDEC_Cantidad;  //4
        $pendiente=$verificar_cantidad->OCOMDEC_Pendiente;  //2
    ...
    public function reportes()
    {
        ...
        $data['combo2'] = $combo2;
        $data['combo3'] = $combo3;
        $this->layout->view('compras/pedido_reporte', $data);
    }
    ...
    public function ver_reporte_pdf($params,$tipo_oper)
    {
        ...
        $fechahoy = date('d/m/Y');
        $listado = $this->pedido_model->buscar_ocompra($tipo_oper,$fechai, $fechaf, $proveedor, $producto, $aprobado, $ingreso);
    ...
    public function ventana_ocompra_factura($ocompra)
    {
        ...
        if ($numero != '') {
            $this->pedido_model->modificar_ocompra_flagRecibido($codigo, $numero);
    ...
    {
        /* Imagen 1 */
        $listado = $this->pedido_model->reporte_ocompra_5_prov_mas_importantes();
        ...
        /* Imagen 2 */
        $listado = $this->pedido_model->reporte_ocompra_monto_x_mes();
        $reg = $listado[0];
        ...
        /* Imagen 3 */
        $listado = $this->pedido_model->reporte_ocompra_cantidad_x_mes();
        $reg = $listado[0];
        ...
        /* Imagen 4 => COMPRAS */
        $listado = $this->pedido_model->reporte_comparativo_compras_ventas('C');
        $reg = $listado[0];
        ...
        echo '<h3>4. Compras</h3>
               ...
               <br />';
        /* Imagen 5 => VENTAS */
        /* $listado=$this->pedido_model->reporte_comparativo_compras_ventas('V'); 
          $reg=$listado[0];
    ...
    {
        $ocompras = $this->pedido_model->listar($tipo_oper);
        $lista = array();
        if (count($ocompras) > 0) {
    ...
    public function registro_ocompras_pdf($tipo_oper, $fecha_ini='', $fecha_fin='',$ruc='', $nombre='')
    {
        ...
        $listado_productos = $this->pedido_model->listar_ocompras_pdf($tipo_oper,$fecha_ini, $fecha_fin,$codigo,$nombre);
    ...
    public function getInfoSendMail(){
        ...
        $ocompraInfo = $this->pedido_model->getOcompraMail($ocompra);
    ...
    public function ventana_cotizacion_correos($codigo) {
        ...
        $datos_presupuesto = $this->pedido_model->obtener_ocompra($codigo);       
        ...
        $data['url_action'] = base_url() . "index.php/compras/pedido/Enviarcorreo";
        $data['hoy'] = $fecha;
        $data['codigo'] = $codigo;
    ...
    public function Enviarcorreo() {
        ...
        for ($i = 0; $i < $size; $i++){
            ...
            $pdfgrabar = 1;
            $uPDF = $this->lib_props->pedido_pdf($codigo, 1, true);
    ...
    public function insertar_pedido($codigoOC){
        ...
        $this->pedido_model->modificar_ocompra($codigoOC, $filter);
        ...
        if (count($detalle) > 0) {
            ...
            $this->lib_props->sendMail(61, $cod_pedido, NULL, "Un pedido ha sido generado desde la aprobacion de un pedido. <b>Serie: $serie-$numero</b>"); # MENU 61 = Órdenes de pedidos
            ...
            if ( $gocompra == true){
                ...
                $ocompra = $this->pedido_model->insertar_ocompra($filterOC);
                $this->configuracion_model->modificar_configuracion($this->somevar['compania'], 3, $numeroOC);
