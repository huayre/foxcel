...
class Movimiento extends Controller{
    ...
    #########################
        ...
        public function datatable_movimiento(){
            ...
            if (count($movimientosInfo) > 0) {
                foreach ($movimientosInfo as $indice => $valor) {
                    ...
                    $posDT=-1;
                    $lista[] = array(
                                        ++$posDT => mysql_to_human($fecha[0]) . " A LAS " . $this->lib_props->formatHours($fecha[1]),
                                        ++$posDT => mysql_to_human($valor->CAJAMOV_FechaRecep),
                                        ++$posDT => $valor->CAJA_CodigoUsuario,
                                        ++$posDT => $valor->CAJA_Nombre,
                                        ++$posDT => $valor->MONED_Simbolo,
                                        ++$posDT => $valor->CAJAMOV_Monto,
                                        ++$posDT => "<span class='bold $colors'>$valor->movimiento</span>",
                                        ++$posDT => $valor->CAJAMOV_Justificacion,
                                        ++$posDT => $btn_modal,
                                        ++$posDT => $btn_eliminar
                                    );
                }
        ...
        public function balance_caja($value='')
        {
            $filter = new stdClass();
            $filter->start = $this->input->post("start");
            $filter->length = $this->input->post("length");
            ...
            $balance        = $this->movimiento_model->balance_cajas($filter);
           
            $total_ingreso  = $balance[0]->total_ingreso;
            $total_salida   = $balance[0]->total_salida;
            $balance_total  = $total_ingreso - $total_salida;
     
            $json = array("total_ingreso" => $total_ingreso, "total_salida" => $total_salida, "balance" => $balance_total);
