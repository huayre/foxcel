...
class Cuentas extends controller{
	...
	###############################
		...
		public function datatable_cuentas($tipo_cuenta = '1'){
			...
			if (count($listado_cuentas) > 0) {
				foreach ($listado_cuentas as $indice => $valor) {
					...
					$estado_formato = obtener_estado_de_cuenta($saldo, $avance, $valor->CPC_Fecha, $valor->CPC_FechaVencimiento);
		...
		public function getCuentaInfo(){
	        ...
	        if ($cuentaResult != NULL){
	        	...
	        	foreach ($cuentaResult as $i => $val) {
					...
					$avance =  ( $listado_pagos == NULL ) ? 0 : $this->pago_model->sumar_pagos($listado_pagos, $val->MONED_Codigo);
	        		...
	        		$listado_pago = $this->cuentaspago_model->listarTodosPagos($val->CUE_Codigo);
			        ...
			        if ($listado_pago != NULL){
			            foreach ($listado_pago as $indice => $valor){
                            ...
                            if ($valor->CPAGC_FlagEstado==5){
                              $btn_cancelar=  "<button class='btn btn-default btn-sm' disabled>
                                                                   Anaulado
                                                                </button>";
                            }
                            else{
                                $btn_cancelar=  "<button class='btn btn-info btn-sm' onclick='modalcancelarpago($valor->CPAGP_Codigo)'>
			                ...
			                $pagosInfo[] = array(
                                                ...
                                                "btncancelar" =>$btn_cancelar,
                                                "estado"  =>$valor->CPAGC_FlagEstado
			                				);
			            }
			        }
	        	}
	    ...
	    public function guardar_pago($tipo_cuenta = 1){
			...
			$tdc = $this->tipocambio_model->getCambio($moneda, $fecha);
	    	# REGISTRAR EL PAGO
		    	$filter = new stdClass();
				...
				$filter->CUENT_CodigoCP = $ctacliente; # CUENTA BANCARIA DEL CLIENTE | PROVEEDOR
				$filter->CUENT_CodigoEmpresa = $micta; # MI CUENTA BANCARIA
			...
			# OBTENER INFORMACION DE LA CUENTA Y ASOCIAR EL PAGO
				...
				if ($cuentaInfo != NULL){
					...
					$estado = ($cuentaInfo[0]->CUE_Monto > $monto) ? "A" : "C";
			        ...
			        if ($caja != "" && $caja != NULL){
			            ...
			            $filter->CAJAMOV_MovDinero = ($tipo_cuenta == 1) ? 1 : 2; # (V:1) = INGRESO | (C:2) = EGRESO
	...
	public function darBajaPago()
    {
        $motivocancelar = $this->input->post('motivocancelar');
        $codigopago =  $this->input->post('codigopago');

        $this->cuentaspago_model->camabiarestado($codigopago);
        $this->cuentaspago_model->actualizarMotivo($motivocancelar,$codigopago);
        $json = array("result" => "success");
        echo json_encode($json);
    }
}
