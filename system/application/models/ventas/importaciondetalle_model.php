<?php
class Importaciondetalle_model extends Model{
    var $somevar;
...
WHERE i.OCOMP_Codigo = 66 AND IMPORDEC_FlagEstado= 1 ORDER BY IMPORDEP_Codigo*/
    ...
    /**********METODOS ADICIONALES *********/
    public function insertarDetalles($filter=null)
    {
        $this->db->insert('cji_importaciondetalle',(array)$filter);

    }
    public function detalles($importacion){
        $sql = "SELECT cd. IMPORDEP_CantidadProducto,cd.IMPORDEP_PrecioCompra,cd. IMPORDEP_PrecioVenta,
       pr.PROD_CodigoInterno, pr.PROD_CodigoUsuario, pr.PROD_CodigoOriginal, pr.PROD_FlagBienServicio,
       pr.PROD_Nombre,pr.PROD_Codigo, m.MARCC_CodigoUsuario, m.MARCC_Descripcion FROM cji_importaciondetalle cd 
           INNER JOIN cji_importacion c ON c. IMPOR_Codigo = cd. IMPOR_Codigo 
           INNER JOIN cji_producto pr ON cd.PROD_Codigo = pr.PROD_Codigo 
           LEFT JOIN cji_marca m ON m.MARCP_Codigo = pr.MARCP_Codigo WHERE cd.IMPOR_Codigo = $importacion AND cd.IMPORDEC_FlagEstado = 1";
