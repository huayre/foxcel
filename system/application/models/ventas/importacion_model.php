...
class Importacion_model extends Model {
...
WHERE od.OCOMP_Codigo=$codigo and od.PROD_Codigo=$prod and id.PROD_Codigo=$prod ";
   ...
   ///************metodos adicionales Importacion***************///
    public function insertarnueva_importacion($filter = null) {
        $compania = $this->somevar['compania'];
        $user = $this->somevar ['user'];
    ...
    public function getImportacionDatatable($filter =null)
    {
        ...
        if (isset($filter->searchRuc_proveedor) && trim($filter->searchRuc_proveedor) != ""){
            $where .= " AND e.EMPRC_Ruc LIKE '$filter->searchRuc_proveedor' ";
        }
        if (isset($filter->searchNombre_proveedor) && trim($filter->searchNombre_proveedor) != ""){
            $where .= " AND e.EMPRC_RazonSocial LIKE '%$filter->searchNombre_proveedor%' ";
        }
        if (isset($filter->searchSerie) && trim($filter->searchSerie) != ""){
            $where .= " AND i.IMPOR_Serie = '$filter->searchSerie' ";
        }
        if (isset($filter->searchNumero) && trim($filter->searchNumero) != ""){
            $where .= " AND i.IMPOR_Numero = '$filter->searchNumero'";
        }


        $sql = "SELECT i.IMPOR_Codigo,i.IMPOR_Serie,i.IMPOR_Numero,i.IMPOR_FlagEstado,i.IMPOR_PercepcionSoles, e.EMPRC_RazonSocial as proveedor FROM
                cji_importacion i
                INNER JOIN cji_proveedor p ON i.PROVP_Codigo=p.PROVP_Codigo
                INNER JOIN cji_empresa e ON e.EMPRP_Codigo=p.EMPRP_Codigo
                WHERE i.COMPP_Codigo = $compania
                ".$where."
                GROUP BY i.IMPOR_Codigo
                $order
                $limit
                ";
    ...
    public function listar_impotacion(){
        $sql = "SELECT i.IMPOR_Codigo,i.IMPOR_FlagEstado,i.IMPOR_FechaRegistro,i.IMPOR_PercepcionSoles, e.EMPRC_RazonSocial as proveedor FROM  cji_importacion i
                INNER JOIN cji_proveedor p ON i.PROVP_Codigo=p.PROVP_Codigo
                INNER JOIN cji_empresa e ON e.EMPRP_Codigo=p.EMPRP_Codigo";
    ...
    public function ActualizarEstadoPrecioCosto($codigo){
        $sql = "UPDATE cji_importacion SET IMPOR_FlagEstado = 5 WHERE 	IMPOR_Codigo = $codigo";
        $query = $this->db->query($sql);
