...
class Producto_model extends model{
    ...
    public function obtener_ult_id(){
        $sql = "SELECT PROD_CodigoUsuario FROM cji_producto WHERE PROD_Codigo = (SELECT MAX(PROD_Codigo) AND PROD_FlagBienServicio='B' FROM cji_producto)";
    ...
    public function listar_producto_stockmin( $filter = NULL ){
        ...
        $sql = "SELECT p.PROD_Codigo, p.PROD_Nombre, p.PROD_FlagBienServicio, p.PROD_StockMinimo, p.PROD_StockMaximo, p.PROD_CodigoInterno, p.PROD_CodigoUsuario, p.PROD_CodigoOriginal, p.PROD_Modelo, p.PROD_GenericoIndividual, p.PROD_UltimoCosto, p.PROD_FlagEstado, p.AFECT_Codigo, m.MARCP_Codigo, m.MARCC_CodigoUsuario, m.MARCC_Descripcion, p.FAMI_Codigo, f.FAMI_Descripcion, fb.FABRIC_Descripcion, CONCAT_WS(' - ', um.UNDMED_Simbolo, um.UNDMED_Descripcion) as UNDMED_Simbolo, ap.ALMPROD_Stock
                    ...
                    WHERE pc.COMPP_Codigo = $compania AND p.PROD_FlagEstado = 1 AND p.PROD_StockMinimo >= ap.ALMPROD_Stock
                    $order
                    $limit
                ";
        $query = $this->db->query($sql);
    ...
    public function stockMin( $lista = true ){
        ...
        $sql = "SELECT pc.COMPP_Codigo, p.PROD_CodigoInterno, p.PROD_CodigoUsuario, p.PROD_Nombre, ap.ALMAC_Codigo,
                    ...
                    INNER JOIN cji_almacenproducto ap ON ap.PROD_Codigo = pc.PROD_Codigo AND ap.COMPP_Codigo IN($compania)
                        ...
                        AND pc.COMPP_Codigo IN($compania)
                ";
        $query = $this->db->query($sql);
    ...
    public function nvoCosto($codigo, $costo){
        $sql = "UPDATE cji_producto SET PROD_CostoPromedio = '$costo' WHERE PROD_Codigo = '$codigo'";
        $query = $this->db->query($sql);
    ...
    public function getProductosDatatable($filter = NULL){
       ...
       /* if ( isset($filter->searchProducto) && $filter->searchProducto != "" && strlen($filter->searchProducto) < 4 )
            $where .= " AND (p.PROD_Nombre LIKE '%$filter->searchProducto%' OR p.PROD_CodigoUsuario LIKE '%$filter->searchProducto%') ";
        else
            if ( isset($filter->searchProducto) && $filter->searchProducto != "" && strlen($filter->searchProducto) >= 4 )
        ...
        $sql = "SELECT p.PROD_Codigo,p.PROD_DescripcionBreve, p.PROD_Nombre, p.PROD_FlagBienServicio, p.PROD_StockMinimo, p.PROD_StockMaximo, p.PROD_CodigoInterno, p.PROD_CodigoUsuario, p.PROD_CodigoOriginal, p.PROD_Modelo, p.PROD_GenericoIndividual, p.PROD_UltimoCosto,p.PROD_CostoPromedio, p.PROD_FlagEstado, p.AFECT_Codigo, m.MARCP_Codigo, m.MARCC_CodigoUsuario, m.MARCC_Descripcion, p.FAMI_Codigo, f.FAMI_Descripcion, fb.FABRIC_Descripcion, CONCAT_WS(' - ', um.UNDMED_Simbolo, um.UNDMED_Descripcion) as UNDMED_Simbolo
                    FROM cji_producto p
                    INNER JOIN cji_productocompania pc ON pc.PROD_Codigo = p.PROD_Codigo
                    LEFT JOIN cji_marca m ON m.MARCP_Codigo = p.MARCP_Codigo
                    LEFT JOIN cji_familia f ON f.FAMI_Codigo = p.FAMI_Codigo
                    ...
                    LEFT JOIN cji_productounidad pu ON pu.PROD_Codigo = p.PROD_Codigo
                    LEFT JOIN cji_unidadmedida um ON um.UNDMED_Codigo = pu.UNDMED_Codigo
                    WHERE pc.COMPP_Codigo = $compania AND p.PROD_FlagEstado = 1
                    $where
                    $order
                    $limit
                ";
    ...
    public function listar_productos_general($flagBS = 'B', $number_items = 0, $offset = 20, $producto_busca = '', $comp_select = null)
    {
        ...
        $sql = "SELECT * FROM cji_producto cp
                ...
                WHERE cp.PROD_FlagBienServicio = '$flagBS' AND cp.PROD_FlagEstado = '1' $where ORDER BY cp.PROD_Nombre LIMIT $offset,$number_items";
...
//Inserta proveedores
    ...
    public function buscar_por_nombre_filtros( $keyword, $flag, $compania, $filtro = NULL ){
        ...
        if ( strlen($keyword) < 8 )
            $where .= " AND (p.PROD_Nombre LIKE '%$keyword%' OR p.PROD_CodigoUsuario LIKE '%$keyword%') ";
        else
            $where .= " AND Match(p.PROD_Nombre, p.PROD_CodigoUsuario) AGAINST ('$keyword') ";
    ...
    public function seleccionar_precio_cliente($prodCodigo = NULL, $tipCli = NULL){ // FUNCION PRECIO - Rawil
   ...
   public function nameproducto($codigo){
    ...
    public function getFamilias($flagBS){
        ...
        $sql = "SELECT * FROM cji_familia WHERE FAMI_FlagBienServicio LIKE '$flagBS' AND FAMI_FlagEstado = 1";
        $query = $this->db->query($sql);
    ...
    public function detallesProduccion($id){
        ...
        public function listar_compatibles($codigo){
        $sql = "SELECT PROD_DescripcionBreve as compatibles FROM cji_producto WHERE PROD_Codigo = $codigo";
        $query = $this->db->query($sql);
        ...
        if($query->num_rows > 0){
            ...
            if (trim($data[0]->compatibles) != '')
                return $data[0]->compatibles;
            else
