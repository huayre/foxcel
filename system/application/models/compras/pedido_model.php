...
class Pedido_model extends Model
{
    ...
    public function seguimiento_listar($tipo_oper = 'C', $number_items = '', $offset = ''){
        ...
        $query = $this->db->order_by('OCOMC_Numero', 'desc')->where_not_in('OCOMP_Codigo', '0')->where($where)->get('cji_pedido', $number_items, $offset);
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
            }
    ...
    {
        $where = array('cji_pedido.OCOMC_FlagEstado' => 1, 'cji_pedidodetalle.OCOMDEC_FlagEstado' => 1, 'cji_pedidodetalle.PROD_Codigo' => $producto);
        $this->db->select('cji_pedido.OCOMC_FechaRegistro,cji_pedido.OCOMC_Numero,cji_pedido.PROVP_Codigo,cji_pedidodetalle.OCOMDEC_Cantidad,cji_pedidodetalle.OCOMDEC_Pu,cji_pedidodetalle.OCOMDEC_Total,cji_pedido.FORPAP_Codigo,cji_pedidodetalle.OCOMDEC_Igv');
        $this->db->from('cji_pedido', $number_items, $offset);
        $this->db->join('cji_pedidodetalle', 'cji_pedidodetalle.OCOMP_Codigo=cji_pedido.OCOMP_Codigo');
        $this->db->where($where);
        $this->db->order_by('cji_pedido.OCOMC_Numero', 'desc');
        $query = $this->db->get();
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
    ...
    public function obtener_ocompra($ocompra){
        #(SELECT CONCAT_WS(' ', pp.PERSC_Nombre, pp.PERSC_ApellidoPaterno, pp.PERSC_ApellidoMaterno) FROM cji_persona pp WHERE pp.PERSP_Codigo = oc.OCOMC_MiPersonal LIMIT 1) as mipersonal
        $sql = "SELECT oc.*,
    ...
    public function obtener_ocompra_guia($ocompra)
    {
        ...
        $sql = "SELECT * FROM cji_pedido WHERE OCOMP_Codigo = (SELECT OCOMP_Codigo FROM cji_guiarem WHERE GUIAREMP_Codigo = $ocompra)";
        $query = $this->db->query($sql);
    ...
    public function obtener_detalle_ocompra($ocompra){
        ...
        $sql = "SELECT oc.*, pr.PROD_CodigoInterno, pr.PROD_CodigoUsuario, pr.PROD_CodigoOriginal, pr.PROD_FlagBienServicio, pr.PROD_Nombre, pr.FABRIP_Codigo, um.UNDMED_Simbolo, m.MARCC_CodigoUsuario, m.MARCC_Descripcion,
                    ...
                    FROM cji_pedidodetalle oc
                    INNER JOIN cji_pedido o ON o.OCOMP_Codigo = oc.OCOMP_Codigo
                    ...
                    LEFT JOIN cji_lote l ON l.LOTP_Codigo = oc.LOTP_Codigo
                        ...
                        ORDER BY oc.OCOMDEC_Descripcion 
                ";
    ...
    public function verificarStockLote_ocompra($ocompra){
        $sql = "SELECT oc.OCOMDEC_Cantidad, oc.OCOMDEC_Descripcion, pr.PROD_CodigoUsuario, m.MARCC_CodigoUsuario, l.LOTC_Numero, apl.ALMALOTC_Cantidad,
                    ...
                    @pendienteOC := (SELECT SUM(ocd.OCOMDEC_Cantidad)
                        FROM cji_pedido oc
                        INNER JOIN cji_pedidodetalle ocd ON ocd.OCOMP_Codigo = oc.OCOMP_Codigo
    ...
    public function obtener_referencia_compra_by_id_detalle_venta($id_detalle_venta){
        $result = $this->db->from('cji_pedidodetalle')
                        ->where(array("cji_pedidodetalle.OCOMDEP_Codigo" => $id_detalle_venta, "cji_pedidodetalle.OCOMDEC_FlagEstado" => 1))
                        ->join("cji_pedido", "cji_pedido.OCOMP_Codigo = cji_pedidodetalle.OCOMP_Codigo", "LEFT")
                        ->join("cji_proyecto", "cji_proyecto.PROYP_Codigo = cji_pedido.PROYP_Codigo", "LEFT")
                        ->join("cji_cliente", "cji_cliente.CLIP_Codigo = cji_pedido.CLIP_Codigo", "LEFT")
                        ->join("cji_proveedor", "cji_proveedor.PROVP_Codigo = cji_pedido.PROVP_Codigo", "LEFT")
                        ->select('cji_pedidodetalle.*, cji_cliente.*, cji_proveedor.*, cji_proyecto.*')->get()->result();
    ...
    {
        $where = array("cji_pedidodetalle.OCOMP_Codigo" => $ocompra, "cji_pedidodetalle.OCOMDEC_FlagEstado" => "1", "cji_pedidodetalle.OCOMDEC_FlagIngreso" => 0);

        $query = $this->db->from("cji_pedidodetalle")->join("cji_pedido", "cji_pedido.OCOMP_Codigo = cji_pedidodetalle.OCOMP_Codigo")->join("cji_proyecto", "cji_proyecto.PROYP_Codigo = cji_pedido.PROYP_Codigo", "LEFT")->where($where)->select("cji_pedidodetalle.*, cji_pedido.*, cji_proyecto.*")->get();
    ...
    public function obtener_detalle_ocompra_importacion($ocompra)
    {
        ...
        $query = $this->db->from("cji_pedidodetalle")
                            ->join("cji_pedido", "cji_pedido.OCOMP_Codigo = cji_pedidodetalle.OCOMP_Codigo_venta", "LEFT")
                            ->join("cji_proyecto", "cji_pedido.PROYP_Codigo = cji_proyecto.PROYP_Codigo", "LEFT")
                            ->where($where)
                            ->select("cji_pedidodetalle.*, cji_pedido.*, cji_proyecto.*")->get();
    ...
    public function insertar_ocompra($filter = null)
    {
        ...
        $this->db->insert("cji_pedido", (array)$filter);
    ...
    public function modificar_ocompra_flagIngreso($ocompra)
    {
        ...
        $query2 = $this->db->where($where2)->get("cji_pedidodetalle");
        if ($query->num_rows == $query2->num_rows) {
            $this->db->where(array("OCOMP_Codigo" => $ocompra))->update("cji_pedido", array("OCOMC_FlagIngreso" => 1));
        } else {
            $this->db->where(array("OCOMP_Codigo" => $ocompra))->update("cji_pedido", array("OCOMC_FlagIngreso" => 0));
        }
    }
    ...
    {
        $where = array("OCOMP_Codigo" => $ocompra);
        $this->db->where($where)->update("cji_pedido", array("OCOMC_FlagRecibido" => 1, "OCOMC_NumeroFactura" => $numero_factura));
    }
    ...
    public function eliminar($ocompra)
    {
        $where = array("OCOMP_Codigo" => $ocompra);
        $this->db->where($where);
        $this->db->delete('cji_pedidodetalle');
        $where = array("OCOMP_Codigo" => $ocompra);
        $this->db->where($where);
    ...
    public function obtenerOrdenCompra(stdClass $filter = NULL, $offset = '', $number_items = ''){
        ...
        */
        ...
        $sql = "SELECT DATE_FORMAT(o.OCOMC_FechaRegistro, '%d/%m/%Y') fecha, o.*,
                    ...
                    FROM cji_pedido o
                    INNER JOIN cji_pedidodetalle od ON od.OCOMP_Codigo = o.OCOMP_Codigo
                    $union
    ...
    public function getOcompra($filter = NULL){
        ...
        if ($filter->serie != '')
            $where .= ' AND o.OCOMC_Serie=' . $filter->serie;
        ...
        $sql = "SELECT DATE_FORMAT(o.OCOMC_FechaRegistro, '%d/%m/%Y') fecha, o.*,
                    ...
                    LEFT JOIN cji_cotizacion ct ON ct.COTIP_Codigo=o.COTIP_Codigo
                    WHERE o.OCOMC_TipoOperacion = '$filter->tipo_oper' AND o.COMPP_Codigo = '$compania'
                    $where
                    GROUP BY o.OCOMP_Codigo
                    $order
                    $limit
    ...
    {
        return $this->db->from("cji_pedido as oc")
                        ->join("cji_pedidodetalle as ocd", "ocd.OCOMP_Codigo = oc.OCOMP_Codigo")
...
FROM cji_pedido o
inner join cji_pedidodetalle od on o.OCOMP_Codigo=od.OCOMP_Codigo
WHERE o.OCOMP_Codigo=$oc and od.OCOMDEC_Pendiente>0";
    ...
    $query = $this->db->query($sql);
     ...
     public function modificar_pendiente_importacion($ocompra,$producto,$cantidad){
        ...
        $sql="
        UPDATE cji_pedidodetalle set OCOMDEC_Pendiente=OCOMDEC_Pendiente-$cantidad
    ...
    public function modificar_pendiente_importacion_by_id_detalle_compra($id_detalle_compra,$cantidad){
     ...
     public function modificar_pendiente($ocompra,$producto,$cantidad){
       /*/ $data      = array("OCOMDEC_Pendiente"=>"OCOMDEC_Pendiente"- $cantidad);
        ...
        $this->db->where($where); 
        $this->db->update('cji_pedidodetalle1',$data);*/
    ...
    public function modificar_pendientecomprobante($ocompra,$cantidad){
        ...
        $sql="
        UPDATE cji_pedidodetalle set OCOMDEC_Pendiente_pago=OCOMDEC_Pendiente_pago-$cantidad
...
public function listar_by_id_proyecto_tipo($id_proyecto, $tipo)
    {
        ...
        $result = $this->db->from("cji_pedido")
                            ->join("cji_pedidodetalle", "cji_pedido.OCOMP_Codigo = cji_pedidodetalle.OCOMP_Codigo")
                            ->where("cji_pedido.PROYP_Codigo", $id_proyecto)
                            ->where("cji_pedido.OCOMC_FlagEstado", 1)
                            ->where("cji_pedidodetalle.OCOMDEC_FlagEstado", 1)
                            ->select("cji_pedidodetalle.OCOMDEP_Codigo")->get()->result();
        ...
        $selects = array('cji_pedido.*', "cji_moneda.*", "cji_persona.*","cji_empresa.*");
        $query = $this->db->from("cji_pedido");

        if($tipo == 'ov') {
            $query->join('cji_cliente', "cji_cliente.CLIP_Codigo = cji_pedido.CLIP_Codigo", "LEFT");

            $selects[] = "cji_cliente.*";
        }else{
            $query->join('cji_proveedor', "cji_proveedor.PROVP_Codigo = cji_pedido.PROVP_Codigo", "LEFT");
            $query->join('cji_pedidodetalle', "cji_pedidodetalle.OCOMP_Codigo = cji_pedido.OCOMP_Codigo", "LEFT");

            $selects[] = "cji_proveedor.*";
            $selects[] = "cji_pedidodetalle.*";
        }

        $query->join('cji_moneda', "cji_moneda.MONED_Codigo = cji_pedido.MONED_Codigo", "LEFT");
        ...
        $query->where('cji_pedido.OCOMC_TipoOperacion', $tipo == 'ov' ? 'V' : 'C');
        $query->where('cji_pedido.OCOMC_FlagEstado', 1);

        if($tipo == 'ov') {
            $query->where('cji_pedido.PROYP_Codigo', $id_proyecto);
        }else {
            $query->select_sum('cji_pedidodetalle.OCOMDEC_Subtotal', 'OCOMDEC_Subtotal_calculado');
            $query->where('cji_pedido.OCOMC_FlagBS', $tipo == "oc" ? 'B' : 'S');
            $query->where_in('cji_pedidodetalle.OCOMP_Codigo_venta', $ordenes);
            $query->group_by('cji_pedidodetalle.OCOMP_Codigo');
            $query->where("cji_pedidodetalle.OCOMDEC_FlagEstado", 1);
        }
    ...
    {
        return $this->db->from("cji_pedidodetalle")
                        ->select("cji_pedidodetalle.OCOMDEC_Pu")
                        ->join("cji_pedido", "cji_pedido.OCOMP_Codigo = cji_pedidodetalle.OCOMP_Codigo")
                        ->where("cji_pedidodetalle.PROD_Codigo", $id_product)
                        ->where("cji_pedidodetalle.OCOMDEC_FlagEstado", 1)
                        ->where("cji_pedido.OCOMC_TipoOperacion", 'C')
                        ->where("cji_pedido.OCOMC_FlagEstado", 1)
                        ->order_by("cji_pedidodetalle.OCOMDEP_Codigo", "DESC")
                        ->limit(1)
                        ->get()->result();   
    }
    ...
    {
        $result = $this->db->from("cji_pedido")
                            ->where("cji_pedido.OCOMP_Codigo", $id)
                            ->get()->result();
  ...
  public function listar_pedidos_finalizados($number_items='',$offset='')
  {
          
    $compania = $this->somevar['compania'];
      
    $this->db->select('*');
        $this->db->from('cji_seguimiento',$number_items,$offset);
        $this->db->where('cji_seguimiento.COMPP_Codigo',$compania);
