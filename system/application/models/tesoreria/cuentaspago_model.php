...
class Cuentaspago_Model extends Model {
    ...
    public function listar($cuenta) {
        ...
        $sql = "SELECT cp.*, p.PAGP_Serie, p.PAGP_Numero, p.PAGC_Monto, p.PAGC_DepoNro, p.PAGC_Trans, p.CHEP_Codigo, p.PAGC_NotaCredito, p.PAGC_FechaOper, p.PAGC_FormaPago, p.PAGC_Obs, p.CUENT_CodigoCP, p.CUENT_CodigoEmpresa, m.MONED_Simbolo, ch.CHEC_Nro, ch.CHEC_FechaEmision, ch.CHEC_FechaVencimiento
    ...
    public function listarTodosPagos($cuenta) {
        ...
        $sql = "SELECT cp.*, p.PAGP_Serie, p.PAGP_Numero, p.PAGC_Monto, p.PAGC_DepoNro, p.PAGC_Trans, p.CHEP_Codigo, p.PAGC_NotaCredito, p.PAGC_FechaOper, p.PAGC_FormaPago, p.PAGC_Obs, p.CUENT_CodigoCP, p.CUENT_CodigoEmpresa, m.MONED_Simbolo, ch.CHEC_Nro, ch.CHEC_FechaEmision, ch.CHEC_FechaVencimiento
                    ...
                    WHERE cp.CUE_Codigo = $cuenta AND (cp.CPAGC_FlagEstado LIKE '1' || cp.CPAGC_FlagEstado LIKE '5')
                    ORDER BY cp.CUE_Codigo DESC
                ";
        $query = $this->db->query($sql);
    ...
    public function camabiarestado($codigo){
        $data = array("CPAGC_FlagEstado" => '5');
        $this->db->where("CPAGP_Codigo", $codigo);
        $this->db->update('cji_cuentaspago', $data);
   }

   public function actualizarMotivo($motivo,$codigopago){
       $data = array("PAGC_Obs" =>$motivo);
       $this->db->where("PAGP_Codigo", $codigopago);
